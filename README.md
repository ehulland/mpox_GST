...
Lastly, at the end of this script there is code to reproduce the counterfactual scenerii for Ecuador and Madagascar where the second foreign language was unavailable. These results are visible in Appendix Figure S24. 
...
**07_mpox_models_LASSO_regression.R**: This code performs the multivariable analysis and variable selection using LASSO regression techniques. Results are shown in Appendix Figures S20 (binary) and S21 (continuous). It only includes imputation Approach 3 (full imputation). This code does use the fully imputed dataset, but instead of using *MASS* for multivariable beta draws, it uses the beta coefficient estimate within the *glmnet* model results. Notably, variables are included in the final multivariable model if they appeared in at least one (of the 10) imputations. This code uses the original data import and cleaning code. 

**08_mpox_models_other_thresholds_binary.R**: This code performs the analysis using two different thresholds for inclusion into the multivariable model on a per-imputation basis. Results are shown in Appendix Figure S22 for a threshold of 0.25 and Figure S23 for no threshold for inclusion. Notably, this analysis was only conducted for the binary outcome, as all eligible, non-collinear variables were included in the multivariable model for our continuous outcome. This analysis employs imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots per outcome: one for the univariable models and one for the multivarible model. This code uses the original data import and cleaning code. 
