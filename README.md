...
Google Trends (GT) data were downloaded from https://trends.google.com/trends/explore for `monkeypox` and `mpox` simultaneously between the period of November 1st, 2022 and December 1st, 2023. This was done for each of the 188 countries and territories included in the analysis. For locations with one or more of the five UN / WHO languages (Appendix Table S1), this search was conducted again using the term `mpox` and the foreign-language term (Appendix Table S2) simultaneously. Covariate data were also acquired at this time.
...
This import and cleaning code brings in all the GST data and covariate data, standardizes location names and spelling, and creates our two outcome variables: our **binary** variable of being an adopter country or not and our **continuous** variable defined as the fraction of all searches that were for `mpox`. Additional covariate processing is tackled here, as described in appendix section 3.1. At the end of this code, we have the following outputs:

 * **full_data**: a dataset with 192 countries and territories included and all covariate data. This dataset is used in imputation Approach 2 and 3 where variable transformations are done per imputation.
* **full_dat**: a subset of full_data with 188 countries and territories included (excluding those missing GDP), and with all transformations done to the covariates prior to building a model. This dataset is used only in imputation Approach 1 (complete case analysis).
* **listwise_del_ctrys**: This is a list of the 154 countries and territories included in the complete case analysis (Approach 1, also referred to as listwise deletion)
* **final_166_ctrys**: This is a list of the 166 countries and territories included in the GAI-imputation only method (Approach 2)
...
**02_mpox_models_foreign_language.R**: This code performs the analysis for the foreign language analysis results shown in Appendix Figures S10 and S11. It only includes imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots per outcome: one for the univariable models (Appendix Figure S10a binary outcome and S11a continuous outcome) and one for the multivariable model with inclusion at 0.1 (Appendix Figure S10b binary outcome and S11b continuous outcome). Importantly, to get the accurate results for this analysis, a *different* preprocessing and importation code needs to be run, as specified at the top of the code: 
...
Lastly, at the end of this script there is code to reproduce the counterfactual scenarios for Ecuador and Madagascar where the second foreign language was unavailable. These results are visible in Appendix Figure S24. 

**03_mpox_models_mpox_case_countries.R**: This code performs the analysis for the set of countries and territories that had every recorded a human or mammalian mpox case since its discovery in humans in the 1970s, results shown in Appendix Figures S12 and S13. It only includes imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots per outcome: one for the univariable models (Appendix Figure S12a binary outcome and S13a continuous outcome) and one for the multivariable model with inclusion at 0.1 (Appendix Figure S12b binary outcome and S13b continuous outcome). This code uses the original data import and cleaning code. 

**04_mpox_models_tobit.R**: This code performs the analysis using a Tobit model for the zero-inflated continuous outcome, with results shown in Appendix Figure S14. It only includes imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots, only for the continuous outcome: one for the univariable models (Appendix Figure S14a) and one for the multivariable model with inclusion at 0.1 (Appendix Figure S14b). This code uses the original data import and cleaning code. 
...
**06a_mpox_models_rockchalk.R**: This code performs the analysis using the *rockchalk* package in R in lieu of the *MASS* package, which allows for seeding of the model results, and thus exact replication. Results are shown in Appendix Figures S15 and S16. It only includes imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots per outcome: one for the univariable models (Appendix Figure S15a binary outcome and S16a continuous outcome) and one for the multivarible model with inclusion at 0.1 (Appendix Figure S15b binary outcome and S16b continuous outcome). This code uses the original data import and cleaning code. 

**06a_mpox_models_mvtnorm.R**: This code performs the analysis using the *mvtnorm* package in R in lieu of the *MASS* package, which allows for seeding of the model results, and thus exact replication. Results are shown in Appendix Figures S17 and S18. It only includes imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots per outcome: one for the univariable models (Appendix Figure S17a binary outcome and S18a continuous outcome) and one for the multivarible model with inclusion at 0.1 (Appendix Figure S17b binary outcome and S18b continuous outcome). This code uses the original data import and cleaning code. 

**07_mpox_models_LASSO_regression.R**: This code performs the multivariable analysis and variable selection using LASSO regression techniques. Results are shown in Appendix Figures S19 (binary) and S20 (continuous). It only includes imputation Approach 3 (full imputation). This code does use the fully imputed dataset, but instead of using *MASS* for multivariable beta draws, it uses the beta coefficient estimate within the *glmnet* model results. Notably, variables are included in the final multivariable model if they appeared in at least one (of the 10) imputations. This code uses the original data import and cleaning code. 

**08_mpox_models_other_thresholds.R**: This code performs the analysis using two different thresholds for inclusion into the multivariable model on a per-imputation basis. Results are shown in Appendix Figure S22a for binary and Figure S22b for continuous for a threshold of 0.25 and Figure S23a for binary and Figure S23b for continuous for no threshold for inclusion. This analysis employs imputation Approach 3 (full imputation). This code uses the same imputation strategy as step 2.3 above, and produces two forest plots per outcome: one for the univariable models and one for the multivarible model. This code uses the original data import and cleaning code. 
