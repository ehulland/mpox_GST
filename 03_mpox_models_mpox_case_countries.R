...
#subset to countries ever having had an mpox case
full_dat_imp<-full_data[!is.na(yr2022) & ever_mpox==1,]
...
jpeg(paste0(dir,'/figures/univar_df_dichotomous_mpoxcase_grouped_OR.jpeg'), height=700, width=1000)
...
jpeg(paste0(dir,'/figures/multivar01_df_dichotomous_mpoxcase_grouped_OR.jpeg'), height=700, width=1000)
ggplot(data=mm1_f_imputation2)+geom_vline(xintercept = 1, col='black',lty=2)+
  ...
  geom_segment(data=mm2_f_imputation2[lower_est<0.01,],aes(x = 0.02, xend = 0.01, y = Variable, col=signif), arrow = arrow(length = unit(0.3, "cm")))

dev.off()

######
### CONTINUOUS OUTCOME, FULL IMPUTATION
#######
#again change the missing "day" data to 999 and then sub out after imputing
#subset to only those countries who ever had an mpox case reported
full_dat_imp<-full_data[!is.na(yr2022) & ever_mpox==1,]
full_dat_imp<-full_dat_imp[NAME %in% full_dat$NAME,]
...
jpeg(paste0(dir,'/figures/univar_df_continuous_mpoxcase_grouped.jpeg'), height=700, width=1000)
...
jpeg(paste0(dir,'/figures/multivar01_df_continuous_mpoxcase_grouped.jpeg'), height=700, width=1000)
